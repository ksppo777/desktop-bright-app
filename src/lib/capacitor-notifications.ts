...
export const requestNotificationPermission = async () => {
  if (Capacitor.isNativePlatform()) {
    try {
      ...
      // Create high importance channel for guaranteed alarm delivery
      await LocalNotifications.createChannel({
        id: "brightstudy_alarm",
        name: "Bright Study 알림",
        description: "학습 시간 및 휴식 시간 알림입니다.",
        importance: 5, // 5 = MAX (highest interruption level)
  ...
  try {
    ...
    for (const alarm of alarms) {
      ...
      for (const dayOfWeek of alarm.days) {
        ...
        const notifId = 10000 + (parseInt(alarm.id) % 100000) * 10 + idOffset; // Ensuring unique ID per alarm per day
